...
setup_window ()
{
  ...
  modifiers = XGetModifierMapping (syswm_info.info.x11.display);
  ...
  for (m = 0; m < 8; m++)
    {
      for (l = 0; l < modifiers->max_keypermod; l++)
        {
          for (i = 0; i < 4; i++)
            {
              KeyCode code = modifiers->modifiermap[m*modifiers->max_keypermod+l];
              KeySym sym = XKeycodeToKeysym (syswm_info.info.x11.display, code, i);
              if (sym >= XK_Meta_L && sym <= XK_Hyper_R)
                meta_mask |= (1 << m);
            }
        }
    }
...
int
state_from_state (unsigned int x11_state)
{
  int state = 0;
  ...
  if (x11_state & meta_mask)
    state |= EV_STATE_META;
  if (x11_state & Button1Mask)
    state |= EV_STATE_BTN_1;
  if (x11_state & Button2Mask)
    state |= EV_STATE_BTN_2;
  if (x11_state & Button3Mask)
    state |= EV_STATE_BTN_3;
  return state;
}

int
input_from_button (unsigned int button, int event_type)
{
  if (event_type == ButtonPress)
    {
      switch (button)
        {
        case Button1:
          return -EV_BTN_1_PRESS;
        case Button2:
          return -EV_BTN_2_PRESS;
        case Button3:
          return -EV_BTN_3_PRESS;
        default:
          return 0;
        }
    }
  else
    {
      switch (button)
        {
        case Button1:
          return -EV_BTN_1_RELEASE;
        case Button2:
          return -EV_BTN_2_RELEASE;
        case Button3:
          return -EV_BTN_3_RELEASE;
        default:
          return 0;
        }
    }
...
handle_event (SDL_Event *e)
{
  if (e->type == SDL_QUIT)
    ...
    {
      XEvent *xe = &(e->syswm.msg->msg.x11.event);
      if (xe->type == KeyPress)
        {
          KeySym sym;
          XLookupString (&xe->xkey, NULL, 0, &sym, NULL);
          int input = input_from_keysym (sym);
          if (input != 0)
            {
              int state = state_from_state (xe->xkey.state);
              if (input > 0)
                state &= ~EV_STATE_SHIFT;
              send_event (EV_INPUT, xe->xkey.x, xe->xkey.y, state, input);
              return 1;
            }
        }
      else if (xe->type == MotionNotify)
        {
          send_event (EV_INPUT,
                      ...
                      state_from_state (xe->xmotion.state),
                      EV_MOVE);
          return 1;
        }
      else if (xe->type == ButtonPress || xe->type == ButtonRelease)
        {
          int input = input_from_button (xe->xbutton.button, xe->type);
          if (input != 0)
            {
              send_event (EV_INPUT,
                          xe->xmotion.x, xe->xmotion.y,
  ...
  while (1)
    {
      ...
      if (nfds == 2)
        {
          if (fds[1].fd == x11_fd && (fds[1].revents & POLLIN))
            events_pending = 1;
